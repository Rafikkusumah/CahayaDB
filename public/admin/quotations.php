...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = [
        'quotation_number' => sanitize($_POST['quotation_number']),
        'quotation_date' => sanitize($_POST['quotation_date']),
    ...
    if (isset($_POST['id']) && $_POST['id']) {
        // Update
        $id = (int)$_POST['id'];
        $sql = "UPDATE quotations SET quotation_number = ?, quotation_date = ?, valid_until = ?, 
        ...
        $params = [
            $data['quotation_number'], $data['quotation_date'], $data['valid_until'],
        ...
        // Insert
        $sql = "INSERT INTO quotations (quotation_number, quotation_date, valid_until, salesperson, 
...
if (isset($_GET['delete'])) {
    $db->prepare("DELETE FROM quotations WHERE id = ?")->execute([(int)$_GET['delete']]);
...
<head>
    ...
    <title>Quotations - CDB Admin</title>
    ...
    <style>
        .line-item-row { transition: all 0.3s; }
        .line-item-row:hover { background-color: #f7fafc; }
    </style>
</head>
<body class="bg-gray-100">
    <div class="flex min-h-screen">
        <aside class="w-64 bg-primary text-white fixed h-full">
            <div class="p-6">
                ...
                <nav class="space-y-2">
                    ...
                    <a href="quotations.php" class="flex items-center space-x-3 px-4 py-3 bg-secondary rounded-lg">
        ...
        <main class="flex-1 ml-64">
            <header class="bg-white shadow-sm px-8 py-4 flex justify-between items-center">
                <h2 class="text-2xl font-bold text-primary">Quotation Management</h2>
                <a href="quotations.php" class="bg-primary text-white px-4 py-2 rounded-lg hover:bg-secondary">
            ...
            <div class="p-8">
                ...
                <div class="bg-white rounded-xl shadow p-6 mb-8">
                    <h3 class="text-lg font-bold text-primary mb-6"><?= $editData ? 'Edit' : 'Buat' ?> Quotation</h3>
                    <form method="POST" id="quotationForm">
                        ...
                        <div class="grid md:grid-cols-3 gap-4 mb-6">
                            <div>
                                <label class="block text-gray-700 font-semibold mb-2">Quotation Number</label>
                                <input type="text" name="quotation_number" required 
                                       value="<?= $editData ? sanitize($editData['quotation_number']) : 'QT-' . date('Ymd') . '-' . rand(100, 999) ?>"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                            </div>
                            <div>
                                <label class="block text-gray-700 font-semibold mb-2">Quotation Date</label>
                                <input type="date" name="quotation_date" required 
                                       value="<?= $editData ? $editData['quotation_date'] : date('Y-m-d') ?>"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                            </div>
                            <div>
                                <label class="block text-gray-700 font-semibold mb-2">Valid Until</label>
                                <input type="date" name="valid_until" required 
                                       value="<?= $editData ? $editData['valid_until'] : date('Y-m-d', strtotime('+30 days')) ?>"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                            </div>
                        </div>
                        ...
                        <div class="grid md:grid-cols-3 gap-4 mb-6">
                            <div>
                                ...
                                <input type="text" name="address" 
                                       value="<?= $editData ? sanitize($editData['address']) : '' ?>"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                            </div>
                            <div>
                                <label class="block text-gray-700 font-semibold mb-2">City</label>
                                <input type="text" name="city" 
                                       value="<?= $editData ? sanitize($editData['city']) : '' ?>"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                            </div>
                            <div>
                                <label class="block text-gray-700 font-semibold mb-2">Zip Code</label>
                                <input type="text" name="zip_code" 
                                       value="<?= $editData ? sanitize($editData['zip_code']) : '' ?>"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                            </div>
                        </div>
                        ...
                        <div class="flex space-x-3">
                            ...
                            <?php if ($editData): ?>
                            <a href="quotations.php" class="bg-gray-500 text-white px-8 py-3 rounded-lg hover:bg-gray-600 transition">Batal</a>
                            <?php endif; ?>
                        </div>
                    </form>
                </div>
                ...
                <div class="bg-white rounded-xl shadow p-6">
                    <h3 class="text-lg font-bold text-primary mb-4">Daftar Quotation</h3>
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600">No. Quotation</th>
                            ...
                            <tbody>
                                ...
                                <tr class="border-t hover:bg-gray-50">
                                    <td class="px-4 py-3 font-semibold text-primary"><?= sanitize($q['quotation_number']) ?></td>
                                    <td class="px-4 py-3"><?= sanitize($q['company_name']) ?></td>
                                    <td class="px-4 py-3"><?= formatDate($q['quotation_date']) ?></td>
                                    <td class="px-4 py-3"><?= formatCurrency(calculateLineItemTotal($q['line_items']) * (1 + ($q['vat_applied'] ? $q['vat_percentage']/100 : 0))) ?></td>
                                    <td class="px-4 py-3">
                                        <div class="flex space-x-2">
                                            <a href="view_quotation.php?id=<?= $q['id'] ?>" class="text-blue-600 hover:text-blue-800" title="View">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="?edit=<?= $q['id'] ?>" class="text-yellow-600 hover:text-yellow-800" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="download_quotation.php?id=<?= $q['id'] ?>" class="text-green-600 hover:text-green-800" title="Download PDF">
                                                <i class="fas fa-file-pdf"></i>
                                            </a>
                                            <a href="?delete=<?= $q['id'] ?>" onclick="return confirm('Hapus quotation ini?')" class="text-red-600 hover:text-red-800" title="Delete">
                                                <i class="fas fa-trash"></i>
                                            </a>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="5" class="px-4 py-8 text-center text-gray-400">Belum ada quotation</td>
                                </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
    ...
    <script>
        function addLineItem() {
            ...
            row.innerHTML = `
                <div class="col-span-4">
                    <input type="text" name="item_name[]" required class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                </div>
                <div class="col-span-2">
                    <input type="number" name="unit_price[]" required step="0.01" class="w-full px-3 py-2 border border-gray-300 rounded-lg" onchange="calculateTotals()">
                </div>
                <div class="col-span-2">
                    <input type="number" name="quantity[]" required step="0.01" class="w-full px-3 py-2 border border-gray-300 rounded-lg" onchange="calculateTotals()">
                </div>
                <div class="col-span-2">
                    <input type="text" name="unit[]" required class="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="pcs/set">
                </div>
                <div class="col-span-1">
                    <input type="text" class="item-total w-full px-3 py-2 bg-gray-100 border rounded-lg text-right" readonly>
                </div>
                <div class="col-span-1">
                    <button type="button" onclick="removeLineItem(this)" class="text-red-500 hover:text-red-700 p-2"><i class="fas fa-trash"></i></button>
                </div>
            `;
        ...
        function toggleVAT() {
            ...
            vatInput.classList.toggle('hidden');
            calculateTotals();
        }
