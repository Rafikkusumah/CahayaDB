...
$vatAmount = $q['vat_applied'] ? $subtotal * ($q['vat_percentage'] / 100) : 0;
    ...
    <div class="info-grid">
        <div class="info-left">
            <div class="info-block">
                ...
                <p><strong>' . sanitize($q['quotation_number']) . '</strong></p>
            </div>
            <div class="info-block">
                <label>Quotation Date</label>
                <p>' . formatDate($q['quotation_date']) . '</p>
            </div>
            <div class="info-block">
                <label>Valid Until</label>
                <p>' . formatDate($q['valid_until']) . '</p>
            </div>
            <div class="info-block">
                <label>Salesperson</label>
                <p>' . sanitize($q['salesperson']) . '</p>
            </div>
        </div>
        <div class="info-right">
            <div class="info-block">
                <label>Company Name</label>
                <p><strong>' . sanitize($q['company_name']) . '</strong></p>
            </div>
            <div class="info-block">
                <label>Address</label>
                <p>' . sanitize($q['address']) . '</p>
            </div>
            <div class="info-block">
                <label>City / Zip Code</label>
                <p>' . sanitize($q['city']) . ' ' . sanitize($q['zip_code']) . '</p>
            </div>
            <div class="info-block">
                <label>Project Description</label>
                <p>' . sanitize($q['project_description']) . '</p>
            </div>
        </div>
    </div>
...
if ($q['vat_applied']) {
    $html .= '
        <div class="totals-row">
            <span>VAT (' . $q['vat_percentage'] . '%)</span>
...
if ($q['notes'] || $q['terms']) {
    ...
    if ($q['notes']) {
        $html .= '
        <h4>Notes:</h4>
        <p>' . sanitize($q['notes']) . '</p>';
    }
    if ($q['terms']) {
        $html .= '
        <h4>Terms & Conditions:</h4>
        <p>' . sanitize($q['terms']) . '</p>';
    }
    $html .= '
    </div>';
}
...
$html .= '
    ...
    <div class="signature-section">
        <div class="signature-box">
            <p style="margin-bottom: 5px;">Jakarta, ' . formatDate($q['quotation_date']) . '</p>
...
generatePDF($html, 'Quotation_' . $q['quotation_number']);